...
- (void)setupPipeline:(MTKView *)view 
{
    ...
    /* load shader sources */
    NSString *shaderSources = [NSString stringWithContentsOfFile:@"../shaders/TriangleTexturedSpinning.metal" 
                                                         encoding:NSUTF8StringEncoding 
                                                            error:&error];
    if (!shaderSources)
    {
        NSLog(@"Failed to loader \"TriangleTexturedSpinning.metal\": %@", error);
        NSLog(@"Ensure you are running from the build directory!");
        [NSApp terminate:nil];
    }
...
- (void)drawInMTKView:(MTKView *)view 
{
    ...
    Uniform *uniformHandle = [_uniformBuffer contents];
    if (uniformHandle)
    {
        uniformHandle->time = [self elapsedTime];
...
- (double)frameTime
{
    return 1e-9 * (double) [self frameTimeNS];
}

- (uint64_t)frameTimeNS
{
    return _currentRenderTimeNS - _lastRenderTimeNS;
}

- (double)elapsedTime
{
    return 1e-9 * (double) [self elapsedTimeNS];
}

- (uint64_t)elapsedTimeNS
{
    return _currentRenderTimeNS - _startTimeNS;
}
